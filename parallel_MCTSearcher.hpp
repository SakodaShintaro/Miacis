...
template <class Var>
std::pair<Move, TeacherType> ParallelMCTSearcher<Var>::think(Position& root) {
    //思考開始時間をセット
...
template <class Var>
ValueType ParallelMCTSearcher<Var>::uctSearch(Position & pos, Index current_index) {
...
template <class Var>
void ParallelMCTSearcher<Var>::parallelUctSearch(Position root) {
    //プレイアウトを繰り返す
...
template <class Var>
bool ParallelMCTSearcher<Var>::isTimeOver() {
    //時間のチェック
...
template <class Var>
bool ParallelMCTSearcher<Var>::shouldStop() {
    return isTimeOver();
//    if (isTimeOver()) {
//        return true;
//    }
...
template <class Var>
std::vector<Move> ParallelMCTSearcher<Var>::getPV() const {
    std::vector<Move> pv;
...
template <class Var>
void ParallelMCTSearcher<Var>::printUSIInfo() const {
...
template <class Var>
std::vector<double> ParallelMCTSearcher<Var>::dirichletDistribution(int32_t k, double alpha) {
...
template <class Var>
int32_t ParallelMCTSearcher<Var>::selectMaxUcbChild(const UctHashEntry & current_node) {
    const auto& child_move_counts = current_node.child_move_counts;
...
template<class Var>
void ParallelMCTSearcher<Var>::onePlay(Position &pos) {
...
template<class Var>
void ParallelMCTSearcher<Var>::clearEvalQueue() {
