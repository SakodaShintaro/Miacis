...
for x in [STEP]:
    for y in [POLICY_LOSS, VALUE_LOSS]:
        # train
        for name, data in zip(labels, train_data):
            plt.plot(data[x], data[y], label=name)
        plt.xlabel(train_labels[x])
        plt.ylabel(train_labels[y])
        if len(labels) > 1:
            plt.legend()
        plt.savefig(f"{dirs[0]}/compare_train_{train_labels[y]}.png", bbox_inches="tight", pad_inches=0.1)
        plt.clf()

        # valid
        for name, data in zip(labels, valid_data):
            plt.plot(data[x], data[y], label=name, marker=".")
        plt.xlabel(valid_labels[x])
        plt.ylabel(valid_labels[y])
        if len(labels) > 1:
            plt.legend()
        plt.savefig(f"{dirs[0]}/compare_valid_{valid_labels[y]}.png", bbox_inches="tight", pad_inches=0.1)
        plt.clf()
        ...
        plt.ylabel(train_labels[y])
        plt.legend()
        plt.savefig(f"{dirs[0]}/compare_train_and_valid_{train_labels[y]}.png", bbox_inches="tight", pad_inches=0.1)
        plt.clf()
