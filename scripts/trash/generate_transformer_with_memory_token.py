...
class TransformerModel(nn.Module):
    def __init__(self, input_channel_num, block_num, channel_num, policy_channel_num, board_size):
        ...
        self.memory_length_ = 10
        self.memory_token_ = torch.nn.Parameter(torch.zeros([self.memory_length_, 1, channel_num]), requires_grad=True)
    ...
    def forward(self, x):
        ...
        memory_token = self.memory_token_.expand((self.memory_length_, batch_size, x.shape[2]))
        x = torch.cat([x, value_token, memory_token], dim=0)
