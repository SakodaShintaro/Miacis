#include"learn_proposed_model.hpp"
...
void learnProposedModel() {
    ...
    HyperparameterLoader settings("learn_proposed_model_settings.txt");
    ...
    //学習推移のログファイル
    std::ofstream train_log("proposed_model_train_log.txt");
    std::ofstream valid_log("proposed_model_valid_log.txt");
    ...
    //評価関数読み込み
    ProposedModel model(options);
    torch::load(model, ProposedModelImpl::DEFAULT_MODEL_NAME);
    model->setGPU(0);

    //学習前のパラメータを出力
    torch::save(model, ProposedModelImpl::MODEL_PREFIX + "_before_learn.model");
    ...
    for (int64_t epoch = 1; global_step < max_step; epoch++) {
        ...
        for (uint64_t step = 0; (step + 1) * batch_size <= train_data.size() && global_step < max_step; step++) {
            ...
            if (global_step % save_interval == 0) {
                //学習中のパラメータを書き出す
                torch::save(model, ProposedModelImpl::MODEL_PREFIX + "_" + std::to_string(global_step) + ".model");
            }
    ...
    std::cout << "finish learnProposedModel" << std::endl;
}