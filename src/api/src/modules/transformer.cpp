#include "../../include/modules/transformer.h"
#include "../../include/modules/transformercoder.h"
#include "../../include/modules/transformerlayer.h"
...
TransformerEncoderLayerImpl::TransformerEncoderLayerImpl(const TransformerEncoderLayerOptions& options_) : options(options_) {
    reset();
}
...
Tensor TransformerEncoderLayerImpl::forward(const Tensor& src, const Tensor& src_mask, const Tensor& src_key_padding_mask) {
...
TransformerDecoderLayerImpl::TransformerDecoderLayerImpl(const TransformerDecoderLayerOptions& options_) : options(options_) {
    reset();
}
...
Tensor TransformerDecoderLayerImpl::forward(Tensor tgt, const Tensor& memory, const Tensor& tgt_mask, const Tensor& memory_mask,
                                            const Tensor& tgt_key_padding_mask, const Tensor& memory_key_padding_mask) {

    Tensor tgt2 = std::get<0>(self_attn(tgt,                  //query
                                        tgt,                  //key
                                        tgt,                  //value
    ...
    tgt2 = std::get<0>(multihead_attn(tgt,                     //query
                                      memory,                  //key
                                      memory,                  //value
...
Tensor TransformerDecoderLayerImpl::activation(const Tensor& input) { return F::relu(input); }

// ========================TransformerEncoderImpl=========================
TransformerEncoderImpl::TransformerEncoderImpl(TransformerEncoderOptions options_) : options(std::move(options_)) { reset(); }
...
void TransformerEncoderImpl::reset_parameters() {
    TORCH_CHECK(layers->size() == options.num_layers(), "TransformerEncoder should have", options.num_layers(),
                " encoder layers, but got ", layers->size());
...
Tensor TransformerEncoderImpl::forward(const Tensor& src, const Tensor& src_mask, const Tensor& src_key_padding_mask) {
...
TransformerDecoderImpl::TransformerDecoderImpl(TransformerDecoderOptions options_) : options(std::move(options_)) { reset(); }
...
void TransformerDecoderImpl::reset_parameters() {
    ...
    TORCH_CHECK(layers->size() == options.num_layers(), "TransformerDecoder should have", options.num_layers(),
...
Tensor TransformerDecoderImpl::forward(const Tensor& tgt, const Tensor& memory, const Tensor& tgt_mask, const Tensor& memory_mask,
                                       const Tensor& tgt_key_padding_mask, const Tensor& memory_key_padding_mask) {

    size_t num_layers = layers->size();
    Tensor output;
    if (num_layers > 0) {
        output = layers->at<TransformerDecoderLayerImpl>(0).forward(tgt, memory, tgt_mask, memory_mask, tgt_key_padding_mask,
                                                                    memory_key_padding_mask);
    }
    for (size_t i = 1; i < num_layers; ++i) {
        output = layers->at<TransformerDecoderLayerImpl>(i).forward(output, memory, tgt_mask, memory_mask, tgt_key_padding_mask,
                                                                    memory_key_padding_mask);
    }
...
// =======================================TransformerImpl================================
TransformerImpl::TransformerImpl(TransformerOptions options_) : options(std::move(options_)) { reset(); }
...
void TransformerImpl::reset() {
    ...
    if (options.custom_encoder().is_empty()) {
        ...
        TransformerEncoder trans_encoder(
            TransformerEncoderOptions(TransformerEncoderLayerOptions(options.d_model(), options.nhead())
    ...
    if (options.custom_decoder().is_empty()) {
        ...
        TransformerDecoder trans_decoder(
            TransformerDecoderOptions(TransformerDecoderLayerOptions(options.d_model(), options.nhead())
                                          .dim_feedforward(options.dim_feedforward())
                                          .dropout(options.dropout()),
...
Tensor TransformerImpl::forward(const Tensor& src, const Tensor& tgt, const Tensor& src_mask, const Tensor& tgt_mask,
                                const Tensor& memory_mask, const Tensor& src_key_padding_mask, const Tensor& tgt_key_padding_mask,
                                const Tensor& memory_key_padding_mask) {

    TORCH_CHECK(src.dim() == 3 && tgt.dim() == 3, "src and tgt should have 3 dimensions, but got ", src.dim(), " and ",
                tgt.dim());

    TORCH_CHECK(src.size(1) == tgt.size(1), "src and tgt should have equal batch size (at dim 1), but got ", src.size(1), " and ",
                tgt.size(1));

    TORCH_CHECK(src.size(2) == options.d_model() && tgt.size(2) == options.d_model(),
                "src and tgt should have same feature size as d_model (at dim 2), but got ", src.size(2), " and ", tgt.size(2),
                " while d_model is ", options.d_model());

    Tensor memory = this->encoder.forward<Tensor>(src, src_mask, src_key_padding_mask);
    Tensor output =
        this->decoder.forward<Tensor>(tgt, memory, tgt_mask, memory_mask, tgt_key_padding_mask, memory_key_padding_mask);
...
Tensor TransformerImpl::generate_square_subsequent_mask(int64_t sz) {
    ...
    TORCH_CHECK(sz >= 0, "Input size must be non-negative to genearte a valid square subsequent mask, but got ", sz);
    ...
    // if IEEE754 is not supported, we use the smallest float number in current platform
        ...
        TORCH_WARN_ONCE("IEEE754 is not supporetd on this platform, generate_square_subsequent_mask will fill "
