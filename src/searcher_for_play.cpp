...
SearcherForPlay::SearcherForPlay(const SearchOptions& search_options)
    ...
    for (int64_t i = 0; i < search_options.gpu_num; i++) {
        ...
        neural_networks_[i].load({ search_options_.model_name1, search_options_.model_name2 }, i,
                                 search_options.search_batch_size, search_options.calibration_kifu_path, search_options.use_fp16);
    }
